...
export const canChooseColorGuard: GameGuard<"chooseColor"> = (context, event) => {
    return !!event.color && [PlayerColor.RED, PlayerColor.YELLOW].includes(event.color) && !!context.players.find(p => p.id === event.playerId) && context.players.find(p => p.color === event.color) === undefined
}
